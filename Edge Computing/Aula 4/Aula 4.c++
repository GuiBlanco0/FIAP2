...
const char *TOPICO_LED = "fiap/edge/aula4/SEUNOME/led"; // <-- NOVO: comando do LED
...
void conectarMQTT()
{
    ...
    mqtt.setCallback(callback); // <-- NOVO: registra o callback
    while (!mqtt.connected())
    {
        ...
        if (mqtt.connect(id.c_str()))
        {
            ...
            mqtt.subscribe(TOPICO_LED); // <-- NOVO: assina o tópico do LED
...
void setup()
{
    ...
    pinMode(LED, OUTPUT);   // <-- NOVO
    digitalWrite(LED, LOW); // <-- NOVO: começa apagado
    // Conexão com o WiFi
    conectarWiFi();
    conectarMQTT();
}
...
void loop()
{
    ...
    char msg[16];
    dtostrf(temp, 4, 1, msg);  // float -> texto
    mqtt.publish(TOPICO, msg); // publica (QoS 0)
